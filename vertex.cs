...
namespace DijkstraShortestPath2
{
    ...
    class DirectedWeightedGraph
    {
        ...
        //used to initialize predecessors
        private readonly int INFINITY = 99999;
        //used to initialize path lengths of all vertices.
        ...
        private void Dijkstra(int s)
        {
            ...
            // set all vertices as temporary with path length as infinity and
            // predecessor as NIL
            for (v = 0; v < n; v++)
            {
            ...
            while (true)
            {
                ...
                if (c == NIL)
                {
                    // there are no temporary vertices left or all temporary
                    // vertices left have path length of infinity
                    // algorithm ends here. End of while loop.
                    return;
                }
                //set status of current vertex
                // as permanent
        ...
        private int TempVertexMinPL()
        {
            ...
            for (int v = 0; v < n; v++)
            {
                // set as new shortest path length if not visited and path length
        ...
        public void FindPaths(String source)
        {
            ...
            // call Dijkstra's algorithm from here.
            Dijkstra(s);
            Console.WriteLine(
"----------------------------");
            ...
            for (int v = 0; v < n; v++)
            {
                ...
                //if path length of any vertex is infinity then vertex is not reachable from source vert ex.
                if (vertexList[v].pathLength == INFINITY)
                    Console.WriteLine("There is no path from " + source + " to " + vertexList[v].name + "\n");
                else
                {
                    // print shortest path.
                    FindPath(s, v);
